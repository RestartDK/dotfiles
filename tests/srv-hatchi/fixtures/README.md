...
`reference-platform.nix` supplies closure-only QEMU facts. It does not select a physical disk. `../install.nix` applies the production storage shape to disposable `/dev/vda` system and `/dev/vdb` data disks. The nginx fixture in `../services.nix` serves separate Ollama, OpenCode, and Glance-agent markers on ports 8000, 8001, and 8002. It never contacts Nana.
