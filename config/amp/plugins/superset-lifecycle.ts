...
type AmpApi = {
  on: (eventName: AmpEventName, handler: (event?: unknown) => unknown | Promise<unknown>) => void;
};
...
function getSessionId(event: unknown): string | undefined {
  ...
  if (thread && typeof thread === "object") {
    return getStringProperty(thread as Record<string, unknown>, ["id", "threadID", "threadId"]);
  }
}
