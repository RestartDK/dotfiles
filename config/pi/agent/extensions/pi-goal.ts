...
function setController(ctx: ExtensionCommandContext | ReplacedSessionContext | undefined): void {
  capturedCommandContext = ctx as ExtensionCommandContext | undefined;
  if (runtimeState) runtimeState.capturedCommandContext = capturedCommandContext;
}
...
function appendState(
  ...
  const entry: GoalStateEntry = {
    ...
    goalId,
    thresholdPercent:
      patch.thresholdPercent ?? current?.thresholdPercent ?? DEFAULT_CONTEXT_THRESHOLD_PERCENT,
    timestamp: Date.now(),
    ...patch,
  };
...
function updateTui(ctx: ExtensionContext, state = runtimeState): void {
  ...
  if (state.status === "active") status += `active ${formatPercent(state.lastContextPercent)}`;
    ...
    status += `budget ${formatPercent(state.lastContextPercent)}`;
...
function buildSummary(ctx: ExtensionContext, state = runtimeState): string {
  ...
  if (state.lastHandoffPrompt)
    lines.push(`Last handoff: ${shortObjective(state.lastHandoffPrompt, 160)}`);

  lines.push("", "Controls: `/goal pause`, `/goal resume`, `/goal handoff`, `/goal clear`.");
  if (!state.capturedCommandContext) {
    lines.push(
      "Automatic new-session handoff controller is not captured. Run `/goal resume` to restore it.",
    );
  }
  return lines.join("\n");
}
...
function startGoal(
  ...
  if (
    ...
    usage.percent >= DEFAULT_CONTEXT_THRESHOLD_PERCENT
  ) {
...
function resumeGoal(pi: ExtensionAPI, ctx: ExtensionCommandContext): string {
  ...
  if (
    usage?.percent !== null &&
    usage?.percent !== undefined &&
    usage.percent >= state.thresholdPercent
  ) {
...
function requestManualHandoff(pi: ExtensionAPI, ctx: ExtensionCommandContext): string {
  ...
  if (!state || state.status === "cleared" || state.status === "complete")
    return "No active goal to hand off.";
  setController(ctx);
  const usage = ctx.getContextUsage();
  updateUsage(state, usage);
...
function appendGoalStateToSessionManager(
  sessionManager: any,
  entry: GoalStateEntry,
  sessionName: string,
): void {
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      ...
      return {
        ...
        details: {
          ...
          controllerAvailable: !!capturedCommandContext,
        },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      objective: Type.String({
        description: "The explicit user objective for the long-running goal.",
      }),
    }),
