...
test("reads and inserts a Kitty paste-event image", () => {
	...
	assert.deepEqual(
		harness.protocol.handleInput(packet(`type=read:status=OK:pw=${password}`)),
		{ consume: true },
	);
	harness.protocol.handleInput(
...
test("supports per-MIME paste listings and text fallback", () => {
	const harness = createHarness();
	harness.protocol.handleInput(
		packet(`type=read:status=OK:loc=primary:pw=${encode("token")}`),
	);
	harness.protocol.handleInput(
		packet(`type=read:status=DATA:mime=${encode("text/plain")}`),
	);
	harness.protocol.handleInput(packet("type=read:status=DONE"));
	const request = lastWrite(harness);
	assert.match(request, /loc=primary/);
	const result = completeContentRequest(harness, request, "text/plain", [
		"hello ",
		"over ssh",
	]);
...
test("the Ctrl+V request only accepts images", () => {
	...
	assert.match(
		listRequest,
		new RegExp(`;${MIME_LIST}${STRING_TERMINATOR.replace("\\", "\\\\")}$`),
	);
	...
	const result = harness.protocol.handleInput(
		packet(`type=read:status=DONE:id=${id}`),
	);
	assert.deepEqual(result, { consume: true });
...
test("rejects image data that does not match its MIME type", () => {
	...
	harness.protocol.handleInput(
		packet(`type=read:status=DATA:mime=${encode("image/png")}`),
	);
	harness.protocol.handleInput(packet("type=read:status=DONE"));
	const result = completeContentRequest(
		harness,
		lastWrite(harness),
		"image/png",
		["not a png"],
...
test("rejects clipboard content above the configured limit", () => {
	...
	harness.protocol.handleInput(
		packet(
			`type=read:status=DATA:id=${id}:mime=${encode("text/plain")}`,
			"123456789",
		),
	);
	assert.deepEqual(harness.notices, [
