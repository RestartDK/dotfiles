...
const IMAGE_MIME_PREFERENCE = ["image/png", "image/jpeg", "image/webp", "image/gif"] as const;
const TEXT_MIME_PREFERENCE = ["text/plain;charset=utf-8", "text/plain", "text/uri-list"] as const;
...
export type ProtocolInputResult = { consume: true } | { data: string } | undefined;
...
function parseClipboardPacket(data: string): ClipboardPacket | undefined {
  ...
  const metadataText = separatorIndex === -1 ? body : body.slice(0, separatorIndex);
  const payloadEncoded = separatorIndex === -1 ? undefined : body.slice(separatorIndex + 1);
...
export class ClipboardPasteProtocol {
  ...
  private readonly sessionPasswordEncoded = Buffer.from(randomUUID(), "utf-8").toString("base64");
  private readonly sessionNameEncoded = Buffer.from("Pi SSH image paste", "utf-8").toString(
    "base64",
  );
  ...
  requestImage(): void {
    if (this.state.phase !== "idle") {
      this.callbacks.notify("A clipboard request is already in progress", "warning");
      return;
    }
  ...
  private handleGhosttyImagePaste(data: string): ProtocolInputResult {
    ...
    if (encodedImage.length > Math.ceil(this.maxClipboardBytes / 3) * 4) {
      this.callbacks.notify(`Clipboard content exceeds ${this.maxClipboardBytes} bytes`, "error");
      return { consume: true };
    }
    ...
    if (!isExpectedImage(image, mimeType)) {
      this.callbacks.notify("Clipboard image data does not match its MIME type", "error");
      return { consume: true };
    }
    try {
      return {
  ...
  private handlePacket(packet: ClipboardPacket): ProtocolInputResult {
    ...
    if (packet.status === "OK") {
      if (this.state.phase === "listing") {
        this.state.passwordEncoded = packet.passwordEncoded ?? this.state.passwordEncoded;
  ...
  private collectMimeTypes(state: ListingState, packet: ClipboardPacket): void {
    const mimeType = packet.mimeEncoded ? decodeBase64Text(packet.mimeEncoded) : undefined;
  ...
  private collectContent(state: ContentState, packet: ClipboardPacket): void {
    ...
    const mimeType = packet.mimeEncoded ? decodeBase64Text(packet.mimeEncoded) : undefined;
    if (mimeType && normalizeMimeType(mimeType) !== normalizeMimeType(state.mimeType)) {
      this.fail("Terminal returned a different clipboard MIME type than requested");
      return;
    }
  ...
  private finishListing(state: ListingState): ProtocolInputResult {
    const imageMimeType = selectMimeType(state.mimeTypes, IMAGE_MIME_PREFERENCE);
    const textMimeType = selectMimeType(state.mimeTypes, TEXT_MIME_PREFERENCE);
    const mimeType = imageMimeType ?? (state.source === "paste-event" ? textMimeType : undefined);
    if (!mimeType) {
      this.clearTimeout();
      this.state = { phase: "idle" };
      this.callbacks.notify(
    ...
    this.sendReadRequest(mimeType, id, state.location, passwordEncoded, nameEncoded);
    this.refreshTimeout();
    return { consume: true };
  }
  ...
  private finishContent(state: ContentState): ProtocolInputResult {
    ...
    if (normalizeMimeType(state.mimeType).startsWith("image/")) {
      ...
      try {
        return {
          data: bracketPaste(this.callbacks.saveImage(content, normalizeMimeType(state.mimeType))),
        };
      } catch (error) {
        this.callbacks.notify(
          `Could not save clipboard image: ${error instanceof Error ? error.message : String(error)}`,
  ...
  private sendReadRequest(
    ...
    this.callbacks.write(`${OSC_PREFIX}${metadata.join(":")};${mimeEncoded}${STRING_TERMINATOR}`);
  }
