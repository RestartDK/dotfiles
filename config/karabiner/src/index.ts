import { map, rule, writeToProfile, toKey, mapSimultaneous } from "karabiner.ts";

writeToProfile(
  "DK",
  [
    rule("Caps Lock -> Escape").manipulators([
      map("⇪").toIfAlone("⎋", {}, { halt: true }).toDelayedAction(toKey("vk_none"), toKey("⎋")),
    ]),

    rule("Left Shift -> Hyper").manipulators([
      map("l⇧")
    ...
    // Home row mods
    rule("Home row mods - ctrl, alt, cmd, shift").manipulators([
      //
      // Four - left hand
      mapSimultaneous(["a", "s", "d", "f"]).toIfHeldDown("l⌃", ["l⌥", "l⌘", "l⇧"]),
      //
      // Three - left hand
      mapSimultaneous(["a", "s", "d"]).toIfHeldDown("l⌃", ["l⌥", "l⌘"]),
      mapSimultaneous(["a", "d", "f"]).toIfHeldDown("l⌃", ["l⌘", "l⇧"]),
      mapSimultaneous(["s", "d", "f"]).toIfHeldDown("l⌥", ["l⌘", "l⇧"]),
      //
      // Two - left hand
      ...
      //
      // Four - right hand
      mapSimultaneous(["j", "k", "l", ";"]).toIfHeldDown("r⌃", ["r⌥", "r⌘", "r⇧"]),
      //
      // Three - right hand
      mapSimultaneous([";", "l", "k"]).toIfHeldDown("r⌃", ["r⌥", "r⌘"]),
      mapSimultaneous([";", "k", "j"]).toIfHeldDown("r⌃", ["r⌘", "r⇧"]),
      mapSimultaneous(["l", "k", "j"]).toIfHeldDown("r⌥", ["r⌘", "r⇧"]),
      //
      // Two - right hand
