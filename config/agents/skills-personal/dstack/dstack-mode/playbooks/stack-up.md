...
**Reply:** endpoint, login URL(s), auth file path, what was seeded, evidence (screenshot/video path or curl proof), any service that needed a retry with its cause, and the teardown command (`process-compose -p <port> down`, then `bin/kill-stray-stack.sh --yes` for orphans). The stack stays up for the demo unless the user says otherwise; say so in the reply.
