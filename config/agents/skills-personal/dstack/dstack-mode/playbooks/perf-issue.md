...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `openai/gpt-5.6-sol`); review the diff. Capture a post-fix trace.
...
For sustained improvement against a metric rather than a one-off fix, use the Hillclimb playbook (`~/.agents/skills/dstack/dstack-mode/playbooks/hillclimb.md`).
