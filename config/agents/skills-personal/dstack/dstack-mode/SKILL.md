...
- Work spanning more than one PR → state the topology before the first commit (each PR standalone to `main`, or a Graphite stack) and keep a roadmap: what each PR changes for the end user on its own, what later PRs own. Propose the split; whether to split is his call.
...
- **Shipping.** The half after Babysit. Independently verify green work, then land the contiguous verified run through the available provider; use Graphite when it is available. `~/.agents/skills/dstack/dstack-mode/playbooks/shipping.md`.
