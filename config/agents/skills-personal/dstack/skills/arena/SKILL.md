...
3. Pick the runners. Use `arena runners` from `~/.agents/skills/dstack/models.md` when present. Otherwise default to one each on `claude-sonnet-4-5`, `gpt-5.6-sol`, `kimi-k3`, `claude-opus-5-thinking-xhigh`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
