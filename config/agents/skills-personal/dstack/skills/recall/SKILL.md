...
Transcripts live at `~/.pi/agent/sessions/<slug>/<timestamp>_<uuid>.jsonl`, where `<slug>` is the absolute workspace path with each "/" turned into "-" (so `/home/you/proj` becomes `-home-you-proj-`). Every line is one JSON event; user/assistant messages have `.message.role`.

Worktree exception: slug directories that are worktrees of the same repo are the same workspace, and they hold most of the history when the user works in worktrees. Include them in scope: match the repo name in the slug (for `~/cobb`, that is `--home-daniel-cobb--` plus every `*worktrees-cobb-*` slug). The do-not-glob rule guards against unrelated projects' private chats, not against your own repo's worktrees.
