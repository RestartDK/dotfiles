...
At the end of the run, before handing back, check the log told the truth. Read this run's session JSONL under `~/.pi/agent/sessions/<cwd-slug>/`. Do not glob across other cwd slugs; that reads unrelated private chats. Walk the log against what actually happened:
