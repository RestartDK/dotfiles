...
Before spawning investigators, list the available MCP servers via the `mcp` tool status call.
...
- spawn via the subagents tool
- `model`: your configured why-synthesizer model (default `anthropic/claude-fable-5:xhigh`)
