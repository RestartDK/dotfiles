...
Prefer no new test over a bad test. A bad test is one that mostly tests mocks, encodes current implementation details, depends on timing or unrelated global state, needs expensive infrastructure for a small fix, or would be deleted immediately after proving the fix. The full keep-or-cut rubric is **principle-tests-earn-their-place**; the regression test must name the production break it catches.
