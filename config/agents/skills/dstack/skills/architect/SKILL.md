...
Use `role: "architect-runners"` with one explicit `member` or zero-based `seat` per configured panel entry. Read the active profile from `/subagents`; there are no skill-local model defaults.
