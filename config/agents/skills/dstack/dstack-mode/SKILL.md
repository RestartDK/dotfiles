...
- **Shipping.** The half after Babysit. Independently verify green work, then land the contiguous verified run through GitHub's native `gh stack merge` for stacks or `gh pr merge` for standalone PRs. Read `references/github-stacks.md` for setup and merge limits. `~/.agents/skills/dstack/dstack-mode/playbooks/shipping.md`.
